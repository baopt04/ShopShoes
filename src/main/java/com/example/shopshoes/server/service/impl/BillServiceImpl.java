...
import com.example.shopshoes.server.dto.response.bill.InvoiceResponse;
...
import com.example.shopshoes.server.entity.VoucherDetail;
...
import com.example.shopshoes.server.infrastructure.exportPdf.ExportFilePdfFormHtml;
...
import com.example.shopshoes.server.repository.NotificationRepository;
import com.example.shopshoes.server.repository.PaymentsMethodRepository;
import com.example.shopshoes.server.repository.ProductDetailGiveBackRepository;
...
public class BillServiceImpl implements BillService {
    ...
    @Override
    public Bill deleteBill(String code){
        Optional<Bill> billOptional = billRepository.findById(code);
        if (billOptional.isPresent()) {
            Bill bill = billOptional.get();
            billHistoryRepository.deleteAllByIdBill(bill.getId());
                    billRepository.delete(bill);
            System.out.println("Deleted Bill Successfully");
                    return bill;
        }else {
            throw new RestApiException(Message.NOT_EXISTS);
        }
    ...
    public Bill CreateCodeBill(String idEmployees) {
        ...
        System.out.println("Code bil " + bill.getCode() + "id " + bill.getId());
...
public boolean updateProduct(CreateBillOfflineRequest request) {
    ...
    public Bill cancelBill(String id, String idEmployees, ChangStatusBillRequest request) {
        ...
        billDetailResponse.forEach(item -> {
            ...
            if (bill.get().getStatusBill() == StatusBill.XAC_NHAN || bill.get().getStatusBill() == StatusBill.CHO_VAN_CHUYEN) {
                productDetail.get().setQuantity(item.getQuantity() + productDetail.get().getQuantity());
        ...
        System.out.println("Trạng thái trước khi hủy" + bill.get().getStatusBill());
