...
function Cart() {
  ...
  useEffect(() => {
    ...
    setTotalBill(totalPrice - discountAmount);
  }, [totalPrice, voucher]);
  ...
  const openListVoucher = () => {
    if (chooseItemCart.length === 0) {
      ...
      toast.warning("Vui lòng chọn sản phẩm cần sử dụng khuyến mãi", {
        autoClose: 3000,
      });
    } else {
  ...
  const payment = () => {
    ...
    if (chooseItemCart.length === 0) {
      ...
      toast.warning(
        "Đơn hàng đã lớn hơn  100 triệu, vui lòng liên hệ với  cửa hàng hoặc chọn lại sản phẩm!"
      );
      return;
    } else {
      if (idAccountLocal === null) {
            ...
            {listVoucher.sort(customSort).map((item, index) => (
              <div
                ...
                className={`item-voucher ${
                  totalBill < item.minimumBill ? "hidden" : ""
                }`}
              >
                ...
                <div style={{ marginLeft: "auto", paddingRight: 30 }}>
                  ...
                  >
                    <Radio
                      value={item.id}
                      disabled={totalBill < item.minimumBill}
                    ></Radio>
                  </Radio.Group>
                </div>
              </div>
